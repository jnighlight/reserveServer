 ...
 * @property integer $admin_email_id
 */
class AdminEmail extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('admin_email_id', 'required'),
			array('admin_email_id', 'numerical', 'integerOnly'=>true),
			array('email', 'length', 'max'=>30),
			array('email', 'email'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('email, admin_email_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('admin_email_id',$this->admin_email_id);
